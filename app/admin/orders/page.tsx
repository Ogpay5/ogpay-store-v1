              ...
              return (
                <div key={order.id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
                  <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                    <div>
                      <p className="text-sm uppercase tracking-[0.25em] text-violet-300">{code}</p>
                      <h3 className="mt-2 text-3xl font-black">${Number(order.total_price || 0).toFixed(2)}</h3>
                      <p className="mt-2 text-sm text-zinc-500">{order.profiles?.email || order.user_id}</p>
                    </div>
